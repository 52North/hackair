...
public class HackAIRDataSourceHarvesterScheduler implements Destroyable {
    private final static Logger LOGGER = LoggerFactory.getLogger(HackAIRDataSourceHarvesterScheduler.class);
    ...
    public boolean removeJob(ScheduledJob taskToSchedule) throws SchedulerException {
        JobDetail details = taskToSchedule.createJobDetails();
        Trigger trigger = taskToSchedule.createTrigger(details.getKey());
        boolean unscheduled = scheduler.unscheduleJob(trigger.getKey());
        LOGGER.debug("Unscheduled job '{}' was successful '{}'?", details.getKey(), unscheduled);
        return unscheduled;
    }
